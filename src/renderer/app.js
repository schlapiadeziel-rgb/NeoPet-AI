...
elements.downloadUpdate.addEventListener("click", async () => { elements.downloadUpdate.classList.add("hidden"); try { await window.neopet.update.download(); } catch (error) { elements.updateStatus.textContent = `下载失败：${error.message}`; } });
elements.installUpdate.addEventListener("click", async () => { if (confirm("将关闭 NeoPet AI 并安装已下载的新版本，继续吗？")) await window.neopet.update.install(); });
